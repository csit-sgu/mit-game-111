...
void ApplyOnSpawn(Context &ctx, Object &obj)
{
	...
	{
		Sound soundShot = LoadSound("Assets/Sounds/shot.mp3");
		PlaySound(soundShot);
	}
}
...
void DrawGameOverScreen(Context &ctx)
{
	...
	// Надпись о проигрыше - можно заменить на "Game over"
	int GameOverTextWidth = MeasureText(GameOverText, 45); 
	// или любую другую, если появится необходимость
	...
	DrawRectangle(0, 0, ctx.screenWidth, ctx.screenHeight, BLACK); 
	// Screen with a black background (Заполняем экран на пустой черный экран)

	//RLAPI void DrawText(const char *text, int posX, int posY, int fontSize, Color color) 
	// Draw text (using default font)
	int posX = (ctx.screenWidth - textWidth) / 2; 
